...
func TestVersionTaskExitsSuccessfully(t *testing.T) {
	...
	result := runStubbedNpmTask(t, constNpmTestVersion)
	tasktestutil.AssertExitCode(t, result, constNpmTestExitSuccess)
...
func TestRunTaskRejectsUnsafeScript(t *testing.T) {
	...
	args := []string{constNpmTestYes, constNpmTestRun, "SCRIPT=dev; rm -rf /"}
	run := tasktestutil.TaskRun{Root: tasktestutil.ModuleRoot(t), Env: npmNvmStubEnv(t), Args: args}
	result := tasktestutil.RunTask(t, run)
...
func makeStubNvmDir(t *testing.T, home string) string {
	t.Helper()

	nvmDir := filepath.Join(home, ".nvm")

	err := os.MkdirAll(nvmDir, constNpmTestFileMode0700)
	if err != nil {
		t.Fatalf("failed to create nvm dir: %v", err)
	}

	err = os.WriteFile(
		filepath.Join(nvmDir, "nvm.sh"),
		[]byte("# nvm stub\n"),
		constNpmTestFileMode0600,
	)
	if err != nil {
...
func writeNvmAndNodeStubs(t *testing.T, binDir string) {
	t.Helper()
...
func writeNvmToolchainStubs(t *testing.T, binDir string) {
	t.Helper()

	writeNvmAndNodeStubs(t, binDir)
	tasktestutil.WriteStub(
		t,
		binDir,
		"npm",
...
func applyNvmStubEnvVars(env []string, binDir, nvmDir string) []string {
	path := tasktestutil.EnvValue(env, constNpmTestPath)

	env = tasktestutil.SetEnv(env, constNpmTestPath, binDir+":"+path)
...
func npmNvmStubEnv(t *testing.T) []string {
	...
	nvmDir := makeStubNvmDir(t, home)

	writeNvmToolchainStubs(t, binDir)
	writeStubBashrc(t, home)

	return applyNvmStubEnvVars(env, binDir, nvmDir)
}
