...
const (
	...
	constTypescriptTestParseTaskfile        = "parse Taskfile: %v"
	constTypescriptTestTasks                = "tasks"
	constTypescriptTestJSON                 = "--json"
	constTypescriptTestDesc                 = "desc"
	constTypescriptTestTaskfileYml          = "Taskfile.yml"
	constTypescriptTestTwo                  = 2
	constTypescriptTestHome                 = "HOME"
	constTypescriptTestTrue                 = "true"
	constTypescriptTestFileMode0644         = 0o644
	constTypescriptTestFileMode0600         = 0o600
	constTypescriptTestSrc                  = "src"
	constTypescriptTestDist                 = "dist"
	constTypescriptTestFileMode0500         = 0o500
	...
	constTypescriptTestReadFmt              = "read %s: %v"
	constTypescriptTestZero                 = 0
	constTypescriptTestOne                  = 1
	constTypescriptTestEmptyString          = ""
	constTypescriptTestFileMode0755         = 0o755
	constTypescriptTestFileMode0700         = 0o700
	...
	constTypescriptTestNoteTsserver         = "Never start tsserver manually"
	constTypescriptTestNoteNotLSP           = "not LSP directly"
	constTypescriptTestNoteTSProto          = "TypeScript Server protocol"
	constTypescriptTestNoteManaged          = "Managed by your editor"
...
func TestTaskListAllJsonIsValid(t *testing.T) {
	...
	result := runTask(t, isolatedEnv(t), constTypescriptTestListAll, constTypescriptTestJSON)
	assertExitCode(t, &result, constTypescriptTestZero)
...
func TestTsserverGuidanceStaysEditorManaged(t *testing.T) {
	...
	for i := range []string{
		constTypescriptTestNoteTsserver,
		constTypescriptTestNoteNotLSP,
		constTypescriptTestNoteTSProto,
		constTypescriptTestNoteManaged,
	} {
		token := []string{
			constTypescriptTestNoteTsserver,
			constTypescriptTestNoteNotLSP,
...
func buildTaskMap(tasksNode *yaml.Node) map[string]*taskfile {
	...
	for i := constTypescriptTestZero; i < len(tasksNode.Content); i += constTypescriptTestTwo {
...
func writeTypeScriptProjectManifestFiles(t *testing.T, dirs *typeScriptFixtureDirs) {
	...
	writeFile(
		...
		`{"scripts":{"build":"tsc"}}`+"\n",
		constTypescriptTestFileMode0644,
	)
	writeFile(
		t,
		filepath.Join(dirs.projectDir, "package-lock.json"),
		"{}\n",
		constTypescriptTestFileMode0600,
	)
	writeFile(
		t,
...
func writeNodeToolStubBinaries(t *testing.T, dirs *typeScriptFixtureDirs, stubBody string) {
	...
	for i := range nodeBinNames {
		writeFile(
			...
			stubBody,
			constTypescriptTestFileMode0500,
		)
	}
}
...
func writePathToolStubBinaries(t *testing.T, dirs *typeScriptFixtureDirs, stubBody string) {
	...
	for i := range toolNames {
		name := toolNames[i]
		writeFile(t, filepath.Join(dirs.binDir, name), stubBody, constTypescriptTestFileMode0500)
	}
}
...
func writeFnmToolchainStub(t *testing.T, home, stubBody string) {
	...
	writeFile(
		...
		stubBody,
		constTypescriptTestFileMode0755,
	)
}
...
func createTypeScriptFixtureDirs(t *testing.T, dirs *typeScriptFixtureDirs) {
	...
	for i := range paths {
		err := os.MkdirAll(paths[i], constTypescriptTestFileMode0700)
		if err != nil {
...
func normalizeTestFileFromParts(t *testing.T, fileValue any, parts []any) testFile {
	...
	if len(parts) != constTypescriptTestTwo {
...
func readmeTaskRowName(row *regexp.Regexp, trimmed string) (string, bool) {
	...
	if len(match) != constTypescriptTestTwo {
		return constTypescriptTestEmptyString, false
	}
...
func mappingValue(node *yaml.Node, name string) *yaml.Node {
	...
	for i := constTypescriptTestZero; i < len(node.Content); i += constTypescriptTestTwo {
...
func readTool(t *testing.T, name string) string {
	...
	if err != nil {
		t.Fatalf(constTypescriptTestReadFmt, name, err)
	}
...
func read(t *testing.T) string {
	...
	if err != nil {
		t.Fatalf(constTypescriptTestReadFmt, path, err)
	}
